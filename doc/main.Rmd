...
```{r}
  ...
  fit <- randomForest(as.factor(outcome)~., data= train_num, mtry =4,
                      importance=TRUE, 
                      ntree=2000)
  
  rf.model = randomForest(Y~., data=trunc.totalData.rforest, subset= trainingIndex, mtry =4, importance=TRUE)
```
